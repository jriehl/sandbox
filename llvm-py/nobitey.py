...
class NoBitey (object):
    ...
    def build_parse_string (self, llvm_types):
    ...
    def handle_abi_casts (self, builder, result):
        if result.type.kind == lc.TYPE_FLOAT:
            ...
            # integration?
            result = builder.fpext(result, llvm_types._double)
        return result

    def build_wrapper_function (self, llvm_function):
        _pyobj_p = llvm_types._pyobject_head_struct_p
        _void_p = _char_p = lc.Type.pointer(llvm_types._int8)
        ...
        _PyArg_ParseTuple = self.target_module.get_or_insert_function(
            lc.Type.function(llvm_types._int32, [_pyobj_p, _char_p], True),
            'PyArg_ParseTuple')
...
def _mk_add_42 (llvm_module, at_type = llvm_types._intp):
