...
class TestANOISpaces(unittest.TestCase):
    def _check_space(self, space: anoi.ANOISpace):
        ...
        self.assertEquals(space.cross(uid0, uid1), NIL)
        self.assertEquals(space.cross(uid1, uid0), NIL)
        self.assertEquals(space.cross(uid0, uid2), NIL)
        self.assertEquals(space.cross(uid2, uid0), NIL)
        self.assertEquals(space.cross(uid1, uid2), NIL)
        self.assertEquals(space.cross(uid2, uid1), NIL)
        empty = tuple()
        self.assertEquals(space.get_content(uid0), empty)
        self.assertEquals(space.get_content(uid1), empty)
        self.assertEquals(space.get_content(uid2), empty)
        test_tuple = tuple(ord(cp) for cp in 'test_tuple')
        space.set_content(uid0, test_tuple)
        self.assertEquals(space.get_content(uid0), test_tuple)
        self.assertEquals(space.get_content(uid1), empty)
        self.assertEquals(space.get_content(uid2), empty)
        space.set_content(uid0, empty)
        space.set_content(uid1, test_tuple)
        self.assertEquals(space.get_content(uid0), empty)
        self.assertEquals(space.get_content(uid1), test_tuple)
        self.assertEquals(space.get_content(uid2), empty)
        space.cross_equals(uid0, uid1, uid2)
        self.assertEquals(space.cross(uid0, uid1), uid2)
        self.assertEquals(space.cross(uid1, uid0), NIL)
        self.assertEquals(space.cross(uid0, uid2), NIL)
        self.assertEquals(space.cross(uid2, uid0), NIL)
        self.assertEquals(space.cross(uid1, uid2), NIL)
        self.assertEquals(space.cross(uid2, uid1), NIL)
        self.assertEquals(space.get_keys(uid0), (uid1,))
        self.assertEquals(space.get_keys(uid1), empty)
        self.assertEquals(space.get_keys(uid2), empty)
        space.free_uid(uid0)
        space.free_uid(uid1)
        space.free_uid(uid2)
