...
class LLVMTranslator (BytecodeFlowVisitor):
    ...
    def op_LOAD_CONST (self, i, op, arg, *args, **kws):
        ...
        if isinstance(py_val, int):
            ret_val = [lc.Constant.int(bytetype.lsize_t, py_val)]
...
def main (*args):
    import llfuncs
    from llfuncs import doslice, ipow, pymod
    func_lltypes = {
        'doslice' : lc.Type.function(bytetype.li8_ptr, (
                bytetype.li8_ptr, bytetype.lsize_t, bytetype.lsize_t)),
    ...
    if not args:
        ...
        args = func_lltypes.keys()
    llvm_module = lc.Module.new('test_module')
    for arg in args:
        translate_function(getattr(llfuncs, arg), func_lltypes[arg],
                           llvm_module)
    print(llvm_module)
